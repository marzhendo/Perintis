...
export async function calculateBep(input) {
  ...
  return {
    ...
    margin: data.margin_per_unit ?? (input.sellingPrice - input.materialCost),
    bepUnits: data.bep_unit_per_bulan ?? Math.ceil(input.monthlyOp / (input.sellingPrice - input.materialCost)),
    roiMonths: data.estimasi_balik_modal_bulan ?? (input.capital / input.monthlyOp),
  };
}
...
export function getFallbackResult(input) {
  ...
  const monthlyProfit = margin > 0 ? (bepUnits * margin) - input.monthlyOp : 0;
  const roiMonths = monthlyProfit > 0 ? parseFloat((input.capital / (monthlyProfit + input.monthlyOp)).toFixed(1)) : 0;
