...
export default function AuthModal({ isOpen, onClose, onLoginSuccess }) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      onLoginSuccess({ name: 'Fatir Gibran', email: 'fatir@gmail.com' });
      onClose();
    } catch {
      setErrors({ general: 'Gagal masuk dengan Google. Coba lagi.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEmailAuth = async (e) => {
    ...
    try {
      ...
      setSuccess(true);
      await new Promise(r => setTimeout(r, 1200));
      onLoginSuccess({ name: form.name || form.email.split('@')[0], email: form.email });
      onClose();
    } catch {
      setErrors({ general: mode === 'login' ? 'Email atau password salah.' : 'Pendaftaran gagal. Coba lagi.' });
    } finally {
      setLoading(false);
    }
  };
        ...
        {success ? (
          ...
          <div className="space-y-6">
            ...
            <button
              ...
              disabled={loading}
              type="button"
              className="w-full bg-white hover:bg-[#F8ECD2]/30 text-[#171C38] font-semibold text-sm border border-[#E8E8E8] rounded-[18px] py-3 px-4 shadow-sm hover:shadow active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed press"
            >
